...
gold_input_dir = "Ripped\\Weapons\\Gold"
gold_output_dir = "Generated\\Weapons\\Gold"
silver_input_dir = "Ripped\\Weapons\\Silver"
silver_output_dir = "Generated\\Weapons\\Silver"
canvas_size = (64, 64)
...
for filename in os.listdir(gold_input_dir):
    if filename.endswith(('.png', '.jpg', '.jpeg', '.gif')):  # Add more supported extensions if needed
        input_image_path = os.path.join(gold_input_dir, filename)
        output_image_path = os.path.join(gold_output_dir, filename)

        resize_and_position(input_image_path, output_image_path)

for filename in os.listdir(silver_input_dir):
    if filename.endswith(('.png', '.jpg', '.jpeg', '.gif')):  # Add more supported extensions if needed
        input_image_path = os.path.join(silver_input_dir, filename)
        output_image_path = os.path.join(silver_output_dir, filename)
