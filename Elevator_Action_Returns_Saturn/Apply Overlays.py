...
for file_name in os.listdir(generated_dir):
    if file_name.endswith(".png"):
        ...
        apply_overlays_and_save(base_image, {k: overlay_paths[k] for k in ["Gold", "Silver", "Bronze"]}, base_file_name, output_dir)

        # Apply each overlay with an additional No_Overlay
        apply_overlays_and_save(base_image, {k: overlay_paths[k] for k in ["Gold", "Silver", "Bronze"]}, base_file_name, output_dir, True)
        
        # Apply each overlay with an additional Red_No_Overlay
        apply_overlays_and_save(base_image, {k: overlay_paths[k] for k in ["Gold", "Silver", "Bronze"]}, base_file_name, output_dir, False, True)
